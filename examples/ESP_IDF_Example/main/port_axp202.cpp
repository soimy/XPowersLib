...
esp_err_t pmu_init()
{
    ...
    if (power.begin(AXP202_SLAVE_ADDRESS, pmu_register_read, pmu_register_write_byte)) {
    ...
    if (power.begin(bus_handle, CONFIG_PMU_SLAVE_ADDRESS)) {
        ESP_LOGI(TAG, "Init PMU SUCCESS!");
    } else {
        ESP_LOGE(TAG, "Init PMU FAILED!");
        return false;
    ...
    if (power.begin((i2c_port_t)CONFIG_I2C_MASTER_PORT_NUM, CONFIG_PMU_SLAVE_ADDRESS, CONFIG_PMU_I2C_SDA, CONFIG_PMU_I2C_SCL)) {
        ESP_LOGI(TAG, "Init PMU SUCCESS!");
    } else {
        ESP_LOGE(TAG, "Init PMU FAILED!");
        return false;
    ...
    * */
    ...
    power.setChargeTargetVoltage(XPOWERS_AXP202_CHG_VOL_4V1);
...
void pmu_isr_handler()
{
    ...
    if (power.isWdtExpireIrq()) {
        ...
        float ibat = power.getBatteryChargeCurrent();
        float obat = power.getBattDischargeCurrent();
        // ESP_LOGI(TAG, "Battery Voltage:%.2f V, Charge Current:%.2f mA, Discharge Current:%.2f mA", vbat, ibat, obat);

        if (power.isCharging())
        {
            ESP_LOGI(TAG, "Battery Voltage:%.2f V, Charge Current:%.2f mA", vbat, ibat);
        }
        else
        {
            ESP_LOGI(TAG, "Battery Voltage:%.2f V, Discharge Current:%.2f mA", vbat, obat);
        }
